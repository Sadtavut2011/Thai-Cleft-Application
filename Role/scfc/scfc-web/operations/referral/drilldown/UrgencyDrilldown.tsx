...
import { ArrowLeft, Send, AlertTriangle, Eye, Building2, Clock, CheckCircle2 } from 'lucide-react';
...
export function UrgencyDrilldown({ referrals, filter, label, onBack, onSelectReferral }: Props) {
  ...
  const destData = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([name, value]) => ({ name: name.replace('โรงพยาบาล', 'รพ.'), value })).sort((a, b) => b.value - a.value).slice(0, 5);
  }, [filtered]);
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6 pb-12 animate-in fade-in duration-300 font-['IBM_Plex_Sans_Thai']">
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-3">
        ...
        <div className="p-2.5 rounded-xl" style={{ backgroundColor: `${uc.hex}15` }}><AlertTriangle className="w-6 h-6" style={{ color: uc.hex }} /></div>
        <div className="flex-1"><h1 className="text-[#5e5873] text-xl">{label}</h1><p className="text-xs text-gray-500">กรองตามระดับความเร่งด่วน "{uc.label}"</p></div>
        <span className="text-xs text-white px-3 py-1.5 rounded-full shrink-0" style={{ backgroundColor: uc.hex }}>{filtered.length} รายการ</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border-gray-100 shadow-sm rounded-xl">
          <CardHeader className="pb-2"><CardTitle className="text-base text-[#5e5873] flex items-center gap-2"><Send className={cn("w-5 h-5", ICON.text)} /> สัดส่วนสถานะ</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="border-gray-100 shadow-sm rounded-xl">
          <CardHeader className="pb-2"><CardTitle className="text-base text-[#5e5873] flex items-center gap-2"><Building2 className={cn("w-5 h-5", ICON.text)} /> ปลายทางส่งตัว</CardTitle></CardHeader>
          <CardContent><div className="space-y-3">
            {destData.map(h => {
              const maxV = destData[0]?.value || 1;
              const pct = Math.round((h.value / maxV) * 100);
              return (<div key={h.name} className="space-y-1"><div className="flex items-center justify-between"><span className="text-sm text-[#5e5873]">{h.name}</span><span className="text-sm text-[#5e5873]">{h.value}</span></div><div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: uc.hex }}></div></div></div>);
            })}
            {destData.length === 0 && <p className="text-sm text-gray-400 text-center py-6">ไม่มีข้อมูล</p>}
          </div></CardContent>
        </Card>
      </div>

      <Card className="border-gray-100 shadow-sm rounded-xl">
        <CardHeader className="pb-2 flex flex-row items-center justify-between"><CardTitle className="text-base text-[#5e5873] flex items-center gap-2"><Send className={cn("w-5 h-5", ICON.text)} /> รายการส่งตัว — {uc.label}</CardTitle><span className="text-xs text-white bg-[#7367f0] px-2.5 py-1 rounded-full">{filtered.length}</span></CardHeader>
        <CardContent className="p-0"><div className="overflow-x-auto"><Table>
          <TableHeader><TableRow className="bg-[#EDE9FE]/30">
            <TableHead className="text-xs text-[#5e5873]">ผู้ป่วย</TableHead><TableHead className="text-xs text-[#5e5873]">ต้นทาง → ปลายทาง</TableHead><TableHead className="text-xs text-[#5e5873]">สถานะ</TableHead><TableHead className="text-xs w-[60px]"></TableHead>
