  ...
  const appointmentsOnSelectedDate = allPatients
    ...
    .filter((p: any) => {
      ...
      if (scope === 'appointment') {
        ...
        const type = p.type || '';
        const isCorrectType = !['Refer In', 'Refer Out', 'Home Visit', 'Joint Visit', 'Telemed', 'Routine'].includes(type);

        // Strict status check for appointment tab (Waiting, Confirmed, Completed only)
        const status = (p.status || '').toLowerCase();
        const isAllowedStatus = ['pending', 'waiting', 'waiting-doctor', 'waiting_staff',
          'checked-in', 'confirmed', 'accepted',
          'completed', 'treated'].includes(status);

        return isCorrectType && isAllowedStatus;
      }
      ...
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-[32px] h-[32px] relative cursor-pointer hover:opacity-90">
            <Filter onFilterSelect={setStatusFilter} scope={scope === 'appointment' ? 'appointment_strict' : scope} />
          </div>
        </div>
      </div>
                    ...
                    <div className="text-[12px] md:text-[14px] text-slate-500 font-medium flex flex-col gap-0.5 mt-1">
                      {scope === 'appointment' && (
                        <div className="flex items-center gap-2">
                          ...
                          {patient.room && <span className="text-slate-400">ห้อง {patient.room}</span>}
                        </div>
                      )}
                      {scope === 'refer' && (
                      ...
                      {scope === 'homevisit' && (
                        <div className="flex flex-col">
                          ...
                          <span className="text-[#7066a9] mt-0.5">
                            {patient.type === 'Joint Visit' ? 'พร้อม รพ.สต.' : (patient.type === 'Routine' ? 'เยี่ยมตามรอบ' : 'ฝากเยี่ยม')}
                          </span>
                        </div>
                      )}
                      {scope === 'telemed' && (
                  ...
                  <td className="p-2 align-middle text-center md:text-right">
                    {(() => {
                      ...
                      if (scope === 'appointment') {
                        ...
                        else if (['checked-in', 'confirmed', 'accepted'].includes(status)) { colorClass = 'bg-emerald-100 text-emerald-700'; label = 'ยืนยันแล้ว'; }
                        else if (status === 'completed' || status === 'treated') { colorClass = 'bg-blue-100 text-blue-700'; label = 'เสร็จสิ้น'; }
                        else if (status === 'missed') { colorClass = 'bg-red-100 text-red-700'; label = 'ขาดนัด'; }
                      } else if (scope === 'refer') {
                        if (status === 'pending') { colorClass = 'bg-orange-100 text-orange-700'; label = 'รอตอบรับ'; }
                        else if (['referred', 'accepted', 'confirmed'].includes(status)) { colorClass = 'bg-emerald-100 text-emerald-700'; label = 'รับตัวแล้ว'; }
                        ...
                        else if (status === 'completed') { colorClass = 'bg-emerald-100 text-emerald-700'; label = 'เยี่ยมแล้ว'; }
                      } else if (scope === 'telemed') {
...
export default function AppointmentSystem({
  ...
  const handleConfirmAction = () => {
    if (confirmModalData && onUpdatePatient) {
      ...
      // Logic to determine next status based on current status and type
      if (type === 'Telemed') {
        ...
        if (['pending', 'waiting', 'confirmed'].includes(status)) {
          ...
          newStatus = 'completed';
          toast.success("การเยี่ยมเสร็จสิ้น");
        }
      } else if (['Refer In', 'Refer Out'].includes(type)) {
        if (['pending', 'waiting'].includes(status)) {
          newStatus = 'accepted';
          toast.success("ตอบรับการส่งตัวแล้ว");
        } else if (['accepted', 'confirmed'].includes(status)) {
          newStatus = 'completed';
          toast.success("บันทึกการรักษาแล้ว");
        }
      } else {
        // Normal Appointment
        if (['waiting', 'pending', 'waiting-doctor', 'waiting_staff'].includes(status)) {
          newStatus = 'confirmed';
          toast.success("ยืนยันนัดหมายแล้ว");
        } else if (['checked-in', 'confirmed', 'accepted'].includes(status)) {
          newStatus = 'completed';
          toast.success("บันทึกการรักษาเรียบร้อย");
        }
      }

      onUpdatePatient({ ...confirmModalData, status: newStatus });
    }
