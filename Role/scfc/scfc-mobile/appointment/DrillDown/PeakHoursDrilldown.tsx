...
import {
  ...
  ClipboardList, Building2, AlertCircle,
  TrendingUp, Activity,
} from 'lucide-react';
...
export function PeakHoursDrilldown({ appointments, onBack }: Props) {
  const hourlyData = useMemo(() => {
    const slots = Array.from({ length: 12 }, (_, i) => ({ hour: `${String(i + 7).padStart(2, '0')}:00`, count: 0 }));
    appointments.forEach(a => { const h = parseHour(a.time); if (h >= 7 && h <= 18) slots[h - 7].count++; });
    if (slots.every(s => s.count === 0)) { const mock = [3, 8, 12, 10, 6, 4, 5, 7, 9, 5, 3, 2]; slots.forEach((s, i) => { s.count = mock[i]; }); }
    return slots;
  }, [appointments]);
  ...
  const hospitalPeak = useMemo(() => {
    ...
    appointments.forEach(a => {
      ...
      const h = parseHour(a.time); if (h >= 7 && h <= 18) entry.hours.set(h, (entry.hours.get(h) || 0) + 1);
    });
    ...
    <div className="bg-[#F4F0FF]/40 min-h-screen flex flex-col font-['IBM_Plex_Sans_Thai'] pb-20">
      <div className="sticky top-0 w-full bg-[#7066a9] h-[64px] px-4 flex items-center gap-3 shrink-0 z-50 shadow-md">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-white text-[18px] font-bold truncate">วิเคราะห์ช่วงเวลานัดหมาย</h1>
          <p className="text-white/70 text-[14px] truncate">ช่วงเวลาหนาแน่น เตรียมจุดคัดกรอง</p>
        </div>
      ...
      <div className="p-4 space-y-4 flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden">
        ...
        >
          {[
            { label: 'นัดหมายทั้งหมด', value: String(totalFromHours), icon: ClipboardList, iconColor: 'text-[#49358E]', iconBg: 'bg-[#F4F0FF]', border: 'border-[#E3E0F0]' },
            { label: 'เวลาหนาแน่น', value: peakSlot?.hour || '-', icon: Clock, iconColor: 'text-rose-600', iconBg: 'bg-rose-50', border: 'border-rose-100' },
            { label: 'นัดหมาย ณ Peak', value: String(peakSlot?.count || 0), icon: TrendingUp, iconColor: 'text-amber-600', iconBg: 'bg-amber-50', border: 'border-amber-100' },
            { label: 'ช่วงเวลาตรวจ', value: `${hourlyData.filter(h => h.count > 0).length} ชม.`, icon: AlertCircle, iconColor: 'text-emerald-600', iconBg: 'bg-emerald-50', border: 'border-emerald-100' },
          ].map((s, i) => (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] p-4">
          <h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2 mb-3"><Clock className="text-[#7066A9]" size={18} /> กราฟช่วงเวลานัดหมาย</h3>
          <div style={{ width: '100%', height: 200, minWidth: 0 }}>
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={hourlyData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                <defs><linearGradient id="peakGradAptMobile" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#49358E" stopOpacity={0.3} /><stop offset="95%" stopColor="#49358E" stopOpacity={0.03} /></linearGradient></defs>
                ...
                <RechartsTooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E3E0F0', fontSize: 16 }} formatter={(val: any) => [`${val} นัดหมาย`, 'จำนวน']} />
                <Area type="monotone" dataKey="count" stroke="#49358E" fill="url(#peakGradAptMobile)" strokeWidth={2.5} dot={{ fill: '#49358E', r: 3 }} activeDot={{ r: 5, fill: '#49358E' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          {peakSlot && peakSlot.count > 0 && (
            <div className="flex items-center gap-2 mt-3 p-3 rounded-xl bg-[#F4F0FF] border border-[#E3E0F0]">
              ...
              <span className="text-[16px] text-[#37286A]">หนาแน่นที่สุด: <span className="font-bold text-[#49358E]">{peakSlot.hour} น.</span> ({peakSlot.count} นัดหมาย)</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] p-4">
          ...
          <div className="flex items-center gap-2.5 mt-4 p-3 rounded-xl bg-[#F4F0FF] border border-[#E3E0F0]">
            <AlertCircle size={16} className="text-[#49358E] shrink-0" />
            <span className="text-[16px] text-[#37286A]">ช่วงเช้ามีนัดหมายมากที่สุด — ควรจัดเจ้าหน้าที่คัดกรองเพิ่มช่วง <span className="font-bold text-[#49358E]">08:00-10:00 น.</span></span>
          </div>
        </div>
        ...
        {hospitalPeak.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] overflow-hidden">
            <div className="p-4 border-b border-[#F4F0FF]"><h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2"><Building2 className="text-[#7066A9]" size={18} /> ช่วงเวลาหนาแน่นแต่ละ รพ.</h3></div>
            <div className="divide-y divide-[#F4F0FF]">
              {hospitalPeak.map(hp => (
                <div key={hp.name} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0"><span className="text-[16px] font-bold text-[#37286A] block truncate">{hp.name}</span><span className="text-[14px] text-[#7066A9]">{hp.total} นัดหมาย</span></div>
                  <div className="text-right shrink-0"><span className="text-[14px] font-bold text-white bg-[#49358E] px-2.5 py-0.5 rounded-full">{hp.peakHour}</span><p className="text-[14px] text-[#7066A9] mt-0.5">{hp.peakCount} นัด ณ Peak</p></div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Hourly detail */}
        <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] overflow-hidden">
          <div className="p-4 border-b border-[#F4F0FF]"><h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2"><CalendarIcon className="text-[#7066A9]" size={18} /> รายละเอียดรายชั่วโมง</h3></div>
          <div className="divide-y divide-[#F4F0FF]">
            {hourlyData.map(slot => {
