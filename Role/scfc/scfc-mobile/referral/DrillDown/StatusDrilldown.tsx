...
import {
  ArrowLeft, Send, CheckCircle2, Clock, AlertTriangle,
  Building2, PieChart as PieChartIcon, MapPin, Calendar as CalendarIcon,
} from 'lucide-react';
import { cn } from '../../../../../components/ui/utils';
import { PieChart, Pie, Cell } from 'recharts';
import { useDragScroll } from '../../components/useDragScroll';
import { FlatReferral, STATUS_CONFIG, getStatusConfig, getUrgencyConfig } from './shared';
      ...
      <div className="sticky top-0 w-full bg-[#7066a9] h-[64px] px-4 flex items-center gap-3 shrink-0 z-50 shadow-md">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white/70 text-[14px] truncate">
            {filter === 'all' ? 'ภาพรวมการส่งตัวทั้งหมด' : `สถานะ "${getStatusConfig(filter).label}"`}
          </p>
        </div>
      ...
      <div className="p-4 space-y-4 flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        >
          {[
            ...
            { label: 'ฉุกเฉิน', value: stats.emergency, icon: AlertTriangle, iconColor: 'text-[#ea5455]', iconBg: 'bg-rose-50', border: 'border-rose-100' },
          ].map((s, i) => (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] overflow-hidden">
            <div className="p-4 border-b border-[#F4F0FF] flex items-center justify-between">
              <h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2">
                <Building2 className="text-[#7066A9]" size={18} /> ปลายทางส่งตัว (Top 6)
              </h3>
            </div>
                ...
                return (
                  <div key={h.name} className="space-y-1.5">
                    ...
                    <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: filterColor }}></div>
                    </div>
                  </div>
                );
              })}
        ...
        <div>
          <div className="flex items-center justify-between px-1 mb-3">
            <h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2">
              <Send className="text-[#7066A9]" size={18} /> รายการส่งตัว
            </h3>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-[#E3E0F0] text-[#7066A9]">
              ...
              <p className="text-[16px]">ไม่พบรายการส่งตัว</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-3 mt-3 bg-[#F4F0FF]/50 rounded-xl px-3 py-2.5">
                        ...
                        <Send size={14} className="text-[#7066A9] shrink-0 rotate-0" />
