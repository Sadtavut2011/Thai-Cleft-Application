...
import KnowledgeBasePage from '../page/KnowledgeBasePage';
...
export default function MobileLayout() {
  ...
  // Get สมชาย ใจดี (first patient from mock data) as the logged-in patient
  const myPatientData = useMemo(() => PATIENTS_DATA.find(p => p.hn === '64001234') || PATIENTS_DATA[0], []);
  ...
  const [myHealthInitialTab, setMyHealthInitialTab] = useState<'profile' | 'history'>('profile');
  const [myHealthInitialSubTab, setMyHealthInitialSubTab] = useState('info');
    ...
    <div className="min-h-screen bg-slate-100 font-sans p-0 text-slate-800">
      <div className="max-w-7xl mx-auto flex flex-col h-screen">
        ...
        <div className={`transition-all duration-500 ease-in-out flex flex-col h-full w-full overflow-y-auto pb-20 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]`}>
            ...
            {currentView === 'dashboard' && (
                <StandardPageLayout title="ภาพรวม" bypassContentCard={true}>
                    <MobileDashboard 
                        ...
                        onViewHealthInfo={() => {
                            setSelectedHistoryPatient(myPatientData);
                            setMyHealthInitialTab('history');
                            setMyHealthInitialSubTab('info');
                            setCurrentView('my-health');
                        }}
                        onViewKnowledge={() => setCurrentView('knowledge')}
                        onViewCarePlan={() => {
                            setSelectedHistoryPatient(myPatientData);
                            setMyHealthInitialTab('profile');
                            setMyHealthInitialSubTab('care_pathway');
                            setCurrentView('my-health');
                        }}
            ...
            {/* Feature: Knowledge Base (สื่อความรู้) */}
            {currentView === 'knowledge' && (
                <KnowledgeBasePage onBack={() => setCurrentView('dashboard')} />
            )}

            {/* CONDITION: View = My Health (Patient's own profile/history — read-only) */}
            {currentView === 'my-health' && selectedHistoryPatient && (
                <StandardPageLayout title="ข้อมูลของฉัน" bypassContentCard={true} onBack={() => setCurrentView('dashboard')}>
                    <PatientDetailView 
                        patient={selectedHistoryPatient} 
                        onBack={() => setCurrentView('dashboard')}
                        readOnly={true}
                        initialTab={myHealthInitialTab}
                        initialSubTab={myHealthInitialSubTab}
                    />
                </StandardPageLayout>
            )}
