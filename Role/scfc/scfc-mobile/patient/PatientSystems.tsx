...
export default function PatientSystems({ patients, onSelectPatient, onAddPatient }: PatientsViewProps) {
  ...
  const filteredPatients = allSystemPatients.filter(p => {
    ...
    // 5. New Filters: Province & Hospital
                   ...
                   <div className="flex flex-col min-w-0 justify-center flex-1">
                     ...
                     <div className="flex items-center justify-between mt-0.5">
                       ...
                       {(() => {
                         ...
                         return (
                           <span className={cn(
                             "inline-flex items-center text-white border-none gap-1 px-2 py-0.5 text-[10px] md:text-xs rounded-full shadow-sm",
                             statusInfo.bg
                           )}>
                             <CheckCircle className="w-3 h-3" strokeWidth={3} /> {statusInfo.label}
                           </span>
                         );
                       })()}
                     </div>
