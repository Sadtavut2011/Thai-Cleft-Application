...
import PCUWorkSystems from '../dashboard/PCUWorkSystems'; // This is "My Work"
...
import { HomeVisitForm } from '../home-visit/forms/VisitForm';
...
export default function MobileLayout() {
    ...
    const handleNavigateToSystem = (system: string, data?: any) => {
        if (system === 'telemed-system') {
            ...
            setSelectedAppointment(data); // Set context for saving later
            setReturnView('calendar');
    ...
    return (
        <div className="min-h-screen bg-slate-100 font-sans p-0 text-slate-800">
            <div className="max-w-7xl mx-auto flex flex-col h-screen">
                ...
                <div id="mobile-bottom-navigation" className={`fixed bottom-0 left-0 w-full bg-white z-50 rounded-t-[24px] shadow-[0px_-6px_4px_0px_rgba(0,0,0,0.04)] border-t-2 border-[#d2cee7] flex-row items-center pb-[20px] pt-[12px] px-[12px] flex`}>
                    ...
                    <nav className="flex flex-row w-full gap-4">
                        ...
                        <button
                            ...
                            className={`flex-1 flex flex-col items-center gap-[8px] p-0 rounded-xl transition-all ${currentView === 'patients'
                                    ? 'text-[#49358e]'
                                    : 'text-[#b8aeea]'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 flex flex-col items-center gap-[8px] p-0 rounded-xl transition-all ${currentView === 'referrals'
                                    ? 'text-[#49358e]'
                                    : 'text-[#b8aeea]'
                                }`}
                        >
                        ...
                        <button
                            onClick={() => {
                                // Navigate to PCU Appointment System (filtered tabs: Appointment, Home Visit, Tele-med)
                                setCurrentView('calendar');
                                setSelectedPatient(null);
                                setSelectedHistoryPatient(null);
                            }}
                            className={`flex-1 flex flex-col items-center gap-[8px] p-0 rounded-xl transition-all ${currentView === 'calendar'
                                    ? 'text-[#49358e]'
                                    : 'text-[#b8aeea]'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 flex flex-col items-center gap-[8px] p-0 rounded-xl transition-all ${currentView === 'chat'
                                    ? 'text-[#49358e]'
                                    : 'text-[#b8aeea]'
                                }`}
                        >
                ...
                <div className={`transition-all duration-500 ease-in-out flex flex-col h-full w-full overflow-y-auto pb-20`}>
                    ...
                    {/* Feature: Home Visit Form (New) */}
                    {currentView === 'home-visit-form' && (
                        <div className="h-full w-full bg-slate-50 relative z-[60]">
                            <HomeVisitForm
                                onBack={() => setCurrentView(returnView)}
                                onSave={(data) => {
                                    // Handle save logic
                                    if (selectedHomeVisit) {
                                        setPatients(prev => {
                                            // Check if patient exists in current state
                                            const exists = prev.some(p => String(p.id) === String(selectedHomeVisit.id));

                                            if (exists) {
                                                // Update existing
                                                return prev.map(p =>
                                                    String(p.id) === String(selectedHomeVisit.id)
                                                        ? { ...p, status: 'completed' }
                                                        : p
                                                );
                                            } else {
                                                // Add new (overrides static data in AppointmentSystem)
                                                return [...prev, { ...selectedHomeVisit, status: 'completed' }];
                                            }
                                        });
                                    }
