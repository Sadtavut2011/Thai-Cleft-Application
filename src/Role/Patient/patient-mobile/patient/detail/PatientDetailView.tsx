...
export const PatientDetailView: React.FC<PatientDetailViewProps> = ({ patient: propPatient, onBack, onEdit, onAddMedicalRecord, onRequestFund, onAction }) => {
    ...
    const patient = {
        ...
        timeline: [
            ...
            { age: '9-18 เดือน', stage: 'ผ่าตัดเย็บซ่อมเพดานปาก', date: '20 ม.ค. 67', status: 'current' },
        ...
        appointmentHistory: [
            { date: '20 ธ.ค. 68', time: '09:00', title: 'ติดตามพัฒนาการ (Follow up)', location: 'แผนกศัลยกรรม รพ.มหาราช', status: 'upcoming', doctor: 'นพ.สมชาย ใจดี', note: 'เตรียมสมุดบันทึกพัฒนาการมาด้วย' },
            { date: '20 ม.ค. 67', time: '08:30', title: 'ผ่าตัดเย็บซ่อมเพดานปาก', location: 'ห้องผ่าตัด รพ.มหาราช', status: 'completed', doctor: 'นพ.สมชาย ใจดี', note: '' },
            { date: '10 ม.ค. 67', time: '10:00', title: 'ตรวจประเมินก่อนผ่าตัด', location: 'OPD ศัลยกรรม', status: 'completed', doctor: 'นพ.สมชาย ใจดี', note: '' }
        ],
        referralHistory: [
        ...
        responsibleHealthCenter: propPatient.responsibleHealthCenter || 'รพ.สต. บ้านสุเทพ'
    };
        ...
        <div className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden font-['IBM_Plex_Sans_Thai'] relative">
            ...
            {isFabOpen && (
                <div className="fixed bottom-[160px] right-4 flex flex-col items-end gap-3 z-50">
                    ...
                    >
                        <span className="font-medium text-sm">ระบบเยี่ยมบ้าน</span>
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 bg-white text-slate-700 px-4 py-2 rounded-full shadow-lg border border-slate-100 hover:bg-slate-50 transition-colors"
                    >
                        <span className="font-medium text-sm">ระบบนัดหมาย</span>
                        <div className="w-8 h-8 rounded-full bg-[#5B4D9D]/10 flex items-center justify-center text-[#5B4D9D]">
                    ...
                    {isWalkInModalOpen && (
                        <CreateAppointmentForm
                            onBack={() => setIsWalkInModalOpen(false)}
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 bg-white text-slate-700 px-4 py-2 rounded-full shadow-lg border border-slate-100 hover:bg-slate-50 transition-colors"
                    >
                        <span className="font-medium text-sm">ระบบส่งตัว</span>
                        <div className="w-8 h-8 rounded-full bg-[#5B4D9D]/10 flex items-center justify-center text-[#5B4D9D]">
                            <Send size={16} />
                        </div>
                    </button>
                    <button
                        ...
                        className="flex items-center gap-2 bg-white text-slate-700 px-4 py-2 rounded-full shadow-lg border border-slate-100 hover:bg-slate-50 transition-colors"
                    >
                        <span className="font-medium text-sm">เพิ่มบันทึกการรักษา</span>
                        <div className="w-8 h-8 rounded-full bg-[#5B4D9D]/10 flex items-center justify-center text-[#5B4D9D]">
                            <Stethoscope size={16} />
                        </div>
                    </button>
                    <button
                        ...
                        className="flex items-center gap-2 bg-white text-slate-700 px-4 py-2 rounded-full shadow-lg border border-slate-100 hover:bg-slate-50 transition-colors"
                    >
                        <span className="font-medium text-sm">ระบบขอทุน</span>
                        <div className="w-8 h-8 rounded-full bg-[#5B4D9D]/10 flex items-center justify-center text-[#5B4D9D]">
                            <FileText size={16} />
                        </div>
                    </button>
