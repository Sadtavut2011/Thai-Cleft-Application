    ...
    <div className="flex flex-col h-full bg-slate-50 font-sans">
      ...
      <div className="bg-white px-4 py-3 sticky top-0 z-20 border-b border-slate-100 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-800 tracking-tight leading-none">รายละเอียดทุน</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{request.id}</p>
          </div>
        </div>
      ...
      <div className="p-4 space-y-4 overflow-y-auto flex-1 pb-24 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        ...
        <Card className="border-slate-200 shadow-sm bg-white rounded-xl overflow-hidden">
          <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
             <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-700">ข้อมูลผู้ป่วย</span>
             </div>
          ...
          <div className="p-4 space-y-3">
             <div className="flex justify-between">
                ...
                <span className="text-xs text-slate-800 font-bold">{request.patientName}</span>
             </div>
             <div className="flex justify-between">
                <span className="text-xs text-slate-500 font-medium">HN</span>
                <span className="text-xs text-slate-800 font-bold">{request.hn}</span>
             </div>
             <div className="flex justify-between">
                <span className="text-xs text-slate-500 font-medium">โรงพยาบาล</span>
                <span className="text-xs text-slate-800 font-bold">{request.hospital}</span>
             </div>
             ...
             <div className="bg-teal-50 p-3 rounded-lg border border-teal-100">
                <div className="flex justify-between items-center mb-2">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-teal-700 uppercase">ยอดเงินที่ขอ</span>
                   </div>
                   <span className="text-lg font-black text-teal-700">฿{request.amount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-start">
                   <span className="text-[10px] text-teal-600 font-medium">แหล่งทุน:</span>
                   <span className="text-[10px] text-teal-800 font-bold text-right max-w-[150px]">{request.fundType}</span>
                </div>
             </div>
          </div>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm bg-white rounded-xl overflow-hidden">
           <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-700">การวินิจฉัย / เหตุผล</span>
           </div>
           <div className="p-4">
        ...
        <Card className="border-slate-200 shadow-sm bg-white rounded-xl overflow-hidden">
           <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-700">เอกสารประกอบ ({request.documents.length})</span>
           </div>
                  ...
                  <div key={i} className="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-100 shadow-sm">
                      <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                              <span className="text-xs font-bold text-slate-700 truncate max-w-[180px]">{doc}</span>
                              <span className="text-[9px] text-slate-400 font-bold uppercase">PDF File</span>
                          </div>
                      </div>
        ...
        {/* Timeline */}
        <Card className="border-slate-200 shadow-sm bg-white rounded-xl overflow-hidden mb-6">
             <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center gap-2">
                <History size={16} className="text-teal-600" />
                <span className="text-xs font-bold text-slate-700">ประวัติการดำเนินการ</span>
             </div>
             <div className="p-4 relative pl-6 space-y-6">
                 <div className="absolute left-[29px] top-6 bottom-6 w-0.5 bg-slate-100"></div>
                 {request.history.map((item, idx) => (
                     <div key={idx} className="relative flex gap-4 z-10">
                         <div className={cn(
                             ...
                             idx === request.history.length - 1 ? "border-teal-500 bg-teal-50" : "border-slate-300"
                         )}></div>
      ...
      {(request.status === 'Pending' || request.status === 'Rejected' || request.status === 'Approved' || request.status === 'Disbursed') && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 pb-8 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] flex gap-3 z-30">
            {request.status === 'Pending' && (
                <>
                    ...
                    <Button 
                        ...
                        className="flex-[2] h-12 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-lg shadow-teal-600/20"
                    >
            ...
            {request.status === 'Approved' && (
                <div className="w-full h-12 flex items-center justify-center bg-blue-50 rounded-xl border border-blue-100 text-blue-600 font-bold">
