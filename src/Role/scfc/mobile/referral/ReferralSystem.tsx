...
export default function ReferralSystem({ onBack, initialHN }: { onBack?: () => void, initialHN?: string }) {
  ...
  const filteredReferrals = MOCK_REFERRALS.filter(ref => {
    ...
    const matchProvince = selectedProvince === 'All' || selectedProvince === 'ทุกจังหวัด' || ref.sourceHospital.includes(selectedProvince); // Simplified mock logic
    const matchHospital = selectedHospital === 'All' || selectedHospital === 'ทุกโรงพยาบาล' || ref.sourceHospital === selectedHospital || ref.destHospital === selectedHospital;
    
    // Status mock mapping could be complex, keeping it simple for now
    const matchStatus = filterStatus === 'all' || 
        (filterStatus === 'pending' && ref.status === 'รอการตอบรับ') ||
        (filterStatus === 'confirmed' && ref.status === 'นัดหมายแล้ว');

    return matchesSearch && matchProvince && matchHospital && matchStatus;
  });
