...
export function EditMedicalRecord({ initialData, onSave, onBack, onDelete }: EditMedicalRecordProps) {
  ...
  useEffect(() => {
    if (initialData?.targetAgeRange) {
      ...
      const monthsMatch = initialData.targetAgeRange.match(/(\d+)\s*เดือน/);
      setAgeDuration({
    ...
    <div className="bg-[#FAFAFA] h-full flex flex-col font-['Montserrat','Noto_Sans_Thai',sans-serif]">
       ...
       <div className="relative bg-white pt-6 pb-4 px-6 shadow-sm z-10 flex-shrink-0 flex items-center gap-4 border-b border-gray-100">
          ...
          <div className="flex flex-col gap-1">
             <h1 className="text-xl font-medium text-[#120d26]">แก้ไขข้อมูลการรักษา</h1>
          </div>
       </div>
         ...
         <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 max-w-lg mx-auto space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <select
                    ...
                    className={cn(
                        "appearance-none px-4 py-1.5 text-sm font-medium rounded-full border-none cursor-pointer outline-none transition-colors",
