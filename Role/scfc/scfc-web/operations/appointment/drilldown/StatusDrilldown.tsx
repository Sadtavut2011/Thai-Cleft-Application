...
import {
  ...
  TrendingUp, Activity, ChevronRight, PieChart as PieChartIcon,
} from 'lucide-react';
...
interface Props {
  ...
  filter: string;   // 'all' | 'confirmed' | 'waiting' | 'completed' | 'cancelled'
  label: string;
  onBack: () => void;
    ...
    <div className="max-w-[1400px] mx-auto space-y-6 pb-12 animate-in fade-in duration-300 font-['IBM_Plex_Sans_Thai']">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-3">
        ...
        <div className="p-2.5 rounded-xl" style={{ backgroundColor: `${filterColor}15` }}>
          <CalendarIcon className="w-6 h-6" style={{ color: filterColor }} />
        </div>
        <div className="flex-1">
          <h1 className="text-[#5e5873] text-xl">{label}</h1>
          <p className="text-xs text-gray-500">
            {filter === 'all' ? 'ภาพรวมนัดหมายทั้งหมดในระบบ' : `รายละเอียดนัดหมายสถานะ "${STATUS_CONFIG[filter]?.label}"`}
          </p>
        </div>
        <span className="text-xs text-white px-3 py-1.5 rounded-full shrink-0" style={{ backgroundColor: filterColor }}>
            ...
            {hospitalData.length > 0 ? (
              <div style={{ minHeight: 220 }}>
                <ResponsiveContainer width="100%" height={220} debounce={50}>
                  <BarChart data={hospitalData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#888' }} />
                    <YAxis tick={{ fontSize: 11, fill: '#888' }} allowDecimals={false} />
                    <RechartsTooltip contentStyle={{ borderRadius: '8px', border: '1px solid #eee', fontSize: '12px' }} formatter={(val: any, _: any, p: any) => [`${val} นัดหมาย`, p.payload.fullName]} />
        ...
        <Card className="border-gray-100 shadow-sm rounded-xl">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between w-full">
              <CardTitle className="text-base text-[#5e5873] flex items-center gap-2">
                <Activity className={cn("w-5 h-5", ICON.text)} /> แยกตามประเภทการรักษา
              </CardTitle>
              <span className="text-xs text-white bg-[#7367f0] px-2.5 py-1 rounded-full shrink-0">
                {treatmentData.length} ประเภท
              </span>
            </div>
          </CardHeader>
          <CardContent>
      ...
      <Card className="border-gray-100 shadow-sm rounded-xl">
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          <CardTitle className="text-base text-[#5e5873] flex items-center gap-2">
            <CalendarIcon className={cn("w-5 h-5", ICON.text)} /> รายการนัดหมาย
          </CardTitle>
