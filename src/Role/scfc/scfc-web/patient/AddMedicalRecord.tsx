...
import { TreatmentSelector } from "../page/TreatmentSelector";
...
export function AddMedicalRecord({ onAdd, onBack, initialData }: AddMedicalRecordProps) {
  ...
  useEffect(() => {
    ...
    if (years === 0 && months === 0 && !formData.calculatedDate) return;
    ...
    if (formData.calculatedDate !== formattedDate || formData.targetAgeRange !== newTargetAgeRange) {
        setFormData(prev => ({ ...prev, targetAgeRange: newTargetAgeRange, calculatedDate: formattedDate }));
    }
  ...
  if (showTreatmentSelector) {
    return <TreatmentSelector initialSelected={formData.treatmentName} onSave={handleTreatmentSelect} onBack={() => setShowTreatmentSelector(false)} />;
  }
  ...
  return (
    <div className="bg-[#FAFAFA] h-full flex flex-col font-['Montserrat','Noto_Sans_Thai',sans-serif]">
       <div className="relative bg-white pt-6 pb-4 px-6 shadow-sm z-10 flex-shrink-0 flex items-center gap-4 border-b border-gray-100">
          ...
          <div className="flex flex-col gap-1"><h1 className="text-xl font-medium text-[#120d26]">เพิ่มข้อมูลการรักษา</h1></div>
       </div>
       <div className="flex-1 overflow-y-auto p-6">
         <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 max-w-lg mx-auto space-y-6">
            <div className="flex items-center justify-between">
                ...
                <Badge className="bg-blue-100 text-blue-600 hover:bg-blue-200 border-none px-4 py-1 text-sm font-medium rounded-full">{formData.status}</Badge>
            </div>
            ...
            <div className="space-y-2">
               ...
               <div onClick={() => setShowTreatmentSelector(true)}>
                 <Input value={formData.treatmentName} readOnly placeholder="แตะเพื่อเลือกการรักษา" className="h-[50px] rounded-xl border-gray-200 bg-white text-[#120d26] focus:ring-[#5669FF] cursor-pointer pointer-events-none" />
               </div>
            </div>
            <div className="space-y-2">
               ...
               <div className="flex gap-4">
                  <div className="flex-1 relative">
                     <Input type="number" min="0" value={ageDuration.years} onChange={(e) => setAgeDuration({...ageDuration, years: e.target.value})} className="h-[50px] rounded-xl border-gray-200 bg-[#F8F9FA] text-[#120d26] focus:ring-[#5669FF] pr-12" placeholder="0" />
                     <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 font-medium">ปี</span>
                  </div>
                  <div className="flex-1 relative">
                     <Input type="number" min="0" max="11" value={ageDuration.months} onChange={(e) => setAgeDuration({...ageDuration, months: e.target.value})} className="h-[50px] rounded-xl border-gray-200 bg-[#F8F9FA] text-[#120d26] focus:ring-[#5669FF] pr-16" placeholder="0" />
            ...
            <div className="space-y-2">
               ...
               <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"><CalendarIcon size={20} /></div>
                  <Input value={formData.calculatedDate} readOnly className="pl-12 h-[50px] rounded-xl border-gray-200 bg-[#F1F3F5] text-[#5e5873] focus:ring-0 cursor-not-allowed border-none" placeholder="ระบบจะคำนวณให้อัตโนมัติ" />
               </div>
            </div>
            <div className="pt-4 space-y-4">
                <Button onClick={() => onAdd(formData)} className="w-full h-[50px] rounded-xl bg-[#7367f0] text-white font-medium hover:bg-[#685dd8] shadow-md shadow-indigo-200 text-[16px] flex items-center justify-center gap-2">
                   <Save className="w-5 h-5" /> บันทึกการแผนการรักษา
                </Button>
                <div className="flex justify-center">
                    <button className="text-red-500 hover:text-red-600 font-medium flex items-center gap-2 text-sm transition-colors" onClick={onBack}>
