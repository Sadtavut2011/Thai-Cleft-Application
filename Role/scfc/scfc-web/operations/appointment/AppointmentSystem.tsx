...
// ===== UI = สีม่วง / Icon = สีน้ำเงิน (#4285f4) =====
...
export function AppointmentSystem({ onBack }: { onBack?: () => void }) {
  ...
  const appointments = useMemo(() => {
    ...
    PATIENTS_DATA.forEach((p) => {
      (p.appointmentHistory || []).forEach((a: any, idx: number) => {
        list.push({
          id: `APT-${p.hn}-${idx}`,
          patientName: p.name, hn: p.hn, image: p.image || '',
          hospital: p.hospital || a.location || '-',
          province: p.province || 'เชียงใหม่',
          clinic: a.title || '-', date: a.date || '-', rawDate: a.rawDate || '',
          time: a.time || '-', type: a.title || '-', status: a.status || 'waiting',
          doctor: a.doctor || '-', room: a.room || '-', note: a.note || '-',
  ...
  // ═══ Dashboard stats (from filteredAppointments — synced with mobile) ═══
  const stats = useMemo(() => ({
    total: filteredAppointments.length,
    pending: filteredAppointments.filter(a => getStatusConfig(a.status) === STATUS_CONFIG.waiting).length,
    confirmed: filteredAppointments.filter(a => getStatusConfig(a.status) === STATUS_CONFIG.confirmed).length,
    completed: filteredAppointments.filter(a => getStatusConfig(a.status) === STATUS_CONFIG.completed).length,
    cancelled: filteredAppointments.filter(a => getStatusConfig(a.status) === STATUS_CONFIG.cancelled).length,
  }), [filteredAppointments]);

  // Pie data (synced with mobile labels/colors)
  ...
  // ═══ Peak Booking Hours (from filteredAppointments) ═══
  ...
  // ═══ Treatment Plan Data (from PATIENTS_DATA.timeline — synced with mobile) ═══
  const treatmentPlanData = useMemo(() => {
    const filteredPatients = PATIENTS_DATA.filter((p: any) => {
      const matchesProvince = provinceFilter === 'ทั้งหมด' || p.province === provinceFilter;
      const matchesHospital = hospitalFilter === 'ทั้งหมด' || (p.hospital || '').includes(hospitalFilter.replace('ทั้งหมด', ''));
  ...
  // ═══ Handle appointment selection from drilldowns ═══
  const handleSelectAppointment = (a: FlatAppointment) => {
    setSelectedAppointment(a);
  };

  // ═══ DRILL-DOWN VIEWS ═══
  if (drilldownView) {
    if (drilldownView.type === 'status') {
      return (
        <StatusDrilldown
          appointments={appointments}
          filter={drilldownView.filter}
          label={drilldownView.label}
          onBack={() => setDrilldownView(null)}
          onSelectAppointment={handleSelectAppointment}
        />
      );
    }
    if (drilldownView.type === 'hospital') {
      return (
        <HospitalDrilldown
          appointments={appointments}
          hospital={drilldownView.hospital}
          fullName={drilldownView.fullName}
          onBack={() => setDrilldownView(null)}
          onSelectAppointment={handleSelectAppointment}
        />
      );
    }
    if (drilldownView.type === 'peakHours') {
      return (
        <PeakHoursDrilldown
          appointments={appointments}
          onBack={() => setDrilldownView(null)}
    ...
    if (drilldownView.type === 'treatment') {
      return (
        <TreatmentDrilldown
          appointments={appointments}
          treatmentName={drilldownView.name}
          treatmentColor={drilldownView.color}
          onBack={() => setDrilldownView(null)}
          onSelectAppointment={handleSelectAppointment}
        />
      );
    ...
    return (
      <AppointmentDetail
        appointment={{
          ...
          province: selectedAppointment.province,
          clinic: selectedAppointment.clinic,
          date: selectedAppointment.rawDate || selectedAppointment.date,
          time: selectedAppointment.time,
          type: selectedAppointment.type,
          status: selectedAppointment.status === 'confirmed' ? 'Confirmed' : selectedAppointment.status === 'cancelled' ? 'Missed' : 'Pending',
          isOverdue: false, hasConflict: false, needsIntervention: false, riskLevel: 'ต่ำ',
        }}
    ...
    <div className="max-w-[1400px] mx-auto space-y-6 pb-12 animate-in fade-in duration-300 font-['IBM_Plex_Sans_Thai']">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className={cn("p-2.5 rounded-xl", ICON.bg)}>
            <CalendarIcon className={cn("w-6 h-6", ICON.text)} />
          </div>
          <div>
            <h1 className="text-[#5e5873] text-xl">ระบบนัดหมาย</h1>
            <p className="text-xs text-gray-500">ติดตามและจัดการนัดหมายทั่วเครือข่าย</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="waiting">รอการยืนยัน</SelectItem>
              <SelectItem value="completed">เสร็จสิ้น</SelectItem>
              <SelectItem value="cancelled">ขาดนัด/ยกเลิก</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      {viewMode === 'dashboard' && (
        <div className="space-y-6">
          ...
          {/* ═══ 5 Stat Cards (synced with mobile) ═══ */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {[
              { label: 'ทั้งหมด', value: stats.total, icon: CalendarIcon, iconColor: ICON.text, iconBg: ICON.bg, borderColor: 'border-gray-100', fk: 'all' },
              { label: 'รอตรวจ', value: stats.pending, icon: Clock, iconColor: 'text-amber-600', iconBg: 'bg-amber-50', borderColor: 'border-amber-100', fk: 'waiting' },
              { label: 'ยืนยันแล้ว', value: stats.confirmed, icon: CheckCircle2, iconColor: 'text-[#49358E]', iconBg: 'bg-[#F4F0FF]', borderColor: 'border-[#E3E0F0]', fk: 'confirmed' },
              { label: 'เสร็จสิ้น', value: stats.completed, icon: TrendingUp, iconColor: 'text-green-600', iconBg: 'bg-green-50', borderColor: 'border-green-100', fk: 'completed' },
              { label: 'ยกเลิก', value: stats.cancelled, icon: XCircle, iconColor: 'text-rose-600', iconBg: 'bg-rose-50', borderColor: 'border-rose-100', fk: 'cancelled' },
            ].map((stat, i) => (
              <Card
                key={i}
                className={cn("shadow-sm rounded-xl hover:shadow-md transition-all cursor-pointer group", stat.borderColor)}
                onClick={() => setDrilldownView({ type: 'status', filter: stat.fk, label: stat.label })}
              >
          ...
          {/* ═══ ข้อมูลนัดหมาย: ช่วงเวลาหนาแน่น + ตามแผนการรักษา + ผู้ป่วยที่อยู่ในการดูแล (synced with mobile) ═══ */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

            {/* Card A: ช่วงเวลาหนาแน่น */}
            <Card className="border-gray-100 shadow-sm rounded-xl flex flex-col">
                ...
                <div className="flex-1" style={{ minHeight: 200 }}>
                  <ResponsiveContainer width="100%" height={200} debounce={50}>
                    <AreaChart data={peakHoursData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                      <defs>
                        <linearGradient id="peakGradAppt" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <RechartsTooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E3E0F0', fontSize: '12px' }} formatter={(val: any) => [`${val} นัดหมาย`, 'จำนวน']} />
                      <Area type="monotone" dataKey="count" stroke="#49358E" fill="url(#peakGradAppt)" strokeWidth={2} dot={{ fill: '#49358E', r: 2.5, strokeWidth: 0 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
                {peakHour && peakHour.count > 0 && (
                  <div className="flex items-center gap-2 mt-2 p-2 rounded-lg bg-[#F4F0FF] border border-[#E3E0F0]">
                    ...
                    <span className="text-xs text-[#37286A]">
                      หนาแน่นที่สุด: <span className="text-[#49358E]">{peakHour.hour} น.</span> ({peakHour.count} นัดหมาย)
                    </span>
                  </div>
                )}
              </CardContent>
              <div className="px-4 pb-4 pt-1 mt-auto">
                <button
                  onClick={() => setDrilldownView({ type: 'peakHours' })}
            ...
            {/* Card B: ตามแผนการรักษา (synced with mobile — from timeline) */}
            <Card className="border-gray-100 shadow-sm rounded-xl flex flex-col">
              <CardHeader className="pb-2 flex flex-row items-center justify-between">
                <CardTitle className="text-base text-[#5e5873] flex items-center gap-2">
                  <ClipboardList className={cn("w-5 h-5", ICON.text)} /> ตามแผนการรักษา
                </CardTitle>
                <span className="text-xs text-white bg-[#7367f0] px-1.5 py-0.5 rounded-full">{treatmentPlanData.length} แผน</span>
              </CardHeader>
              {treatmentPlanData.length > 0 ? (
                <CardContent className="space-y-2.5 flex-1">
                  {treatmentPlanData.slice(0, 2).map((plan) => {
                    ...
                    return (
                      <div key={plan.name} className="p-2.5 rounded-lg bg-gray-50 border border-gray-100">
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: plan.color }}></div>
                            <span className="text-sm text-[#37286A] truncate">{plan.name}</span>
                          </div>
                          <span className="text-xs text-[#7066A9] shrink-0 ml-2">{plan.total} รายการ</span>
                        </div>
                        ...
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="text-[#7066A9]">{pct}%</span>
                        </div>
                      </div>
                    );
                  })}
                  {treatmentPlanData.length > 2 && (
                    <div className="text-center text-xs text-[#7066A9] py-1">+{treatmentPlanData.length - 2} แผนเพิ่มเติม</div>
                  )}
                  <div className="pt-2 mt-1 border-t border-gray-100 flex items-center justify-between px-1">
                    ...
                    <span className="text-sm text-[#7367f0]">{treatmentPlanData.reduce((s, p) => s + p.total, 0)} รายการ</span>
                  </div>
                </CardContent>
              ) : (
                <CardContent className="flex-1 flex flex-col items-center justify-center gap-2 text-center py-6">
                  <ClipboardList size={28} className="text-gray-300" />
                  <p className="text-sm text-[#7066A9]">ไม่พบข้อมูลแผนการรักษา</p>
                  <p className="text-xs text-gray-400">สำหรับตัวกรองที่เลือก</p>
                </CardContent>
              )}
              <div className="px-4 pb-4 pt-1 mt-auto">
                <button
                  onClick={() => { if (treatmentPlanData[0]) setDrilldownView({ type: 'treatment', name: treatmentPlanData[0].name, color: treatmentPlanData[0].color }); }}
                  className="w-full h-[38px] bg-[#EDE9FE] hover:bg-[#DDD6FE] text-[#7367f0] rounded-xl flex items-center justify-center gap-2 text-sm transition-all border border-[#C4BFFA]"
                >
                  <Eye size={16} /> ดูรายละเอียด
                </button>
            ...
            <Card className="border-gray-100 shadow-sm rounded-xl flex flex-col">
              <CardHeader className="pb-2 flex flex-row items-center justify-between">
                ...
                <span className="text-xs text-white bg-[#7367f0] px-2 py-0.5 rounded-full">{PATIENTS_DATA.length} ราย</span>
              </CardHeader>
              <CardContent className="p-0 flex-1">
                <div className="divide-y divide-gray-50">
                  {PATIENTS_DATA.slice(0, 4).map((p: any) => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50/50 transition-colors cursor-pointer">
                        ...
                        {hasPending && <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0"></span>}
                      </div>
                    );
                  })}
                </div>
                {PATIENTS_DATA.length > 4 && (
          ...
          {/* ═══ รายการนัดหมายล่าสุด (kept) ═══ */}
          <Card className="border-gray-100 shadow-sm rounded-xl">
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
              <CardTitle className="text-base text-[#5e5873] flex items-center gap-2">
                <CalendarIcon className={cn("w-5 h-5", ICON.text)} /> รายการนัดหมายล่าสุด
              </CardTitle>
              <Button variant="ghost" size="sm" className="text-[#7367f0] text-sm" onClick={() => setViewMode('list')}>ดูทั้งหมด <ChevronRight size={16} /></Button>
            </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50/50">
                      ...
                      <TableHead className="text-xs text-[#5e5873]">คลินิก/รายการ</TableHead>
                  ...
                  <TableBody>
                    {filteredAppointments.slice(0, 5).map((a) => {
                      ...
                      return (
                        <TableRow key={a.id} className="hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => setSelectedAppointment(a)}>
                          <TableCell>
                            <div className="text-sm text-[#5e5873]">{a.patientName}</div>
                            <div className="text-xs text-gray-400">{a.hn}</div>
                          </TableCell>
                          <TableCell>
      ...
      {viewMode === 'list' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-sm text-gray-600">รายการนัดหมาย</h3>
            <span className="text-xs text-white bg-[#7367f0] px-2.5 py-1 rounded-full">{filteredAppointments.length} รายการ</span>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredAppointments.map((a) => {
                    const sc = getStatusConfig(a.status);
                    return (
                      <TableRow key={a.id} className="hover:bg-[#EDE9FE]/10 cursor-pointer transition-colors" onClick={() => setSelectedAppointment(a)}>
                        <TableCell>
                          <div className="text-sm text-[#5e5873]">{a.patientName}</div>
