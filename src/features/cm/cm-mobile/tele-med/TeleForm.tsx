...
export function TeleForm({ data, onBack, onViewHistory }: TeleFormProps) {
  if (!data) return null;

  return (
    <div className="fixed inset-0 z-[9999] h-[100dvh] w-full bg-slate-50 flex flex-col overflow-hidden font-['IBM_Plex_Sans_Thai'] animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      {/* Header */}
      <div className="w-full bg-[#7066a9] shrink-0 z-[10000] shadow-md pt-[54px] md:pt-0 transition-all relative">
            <div className="h-[64px] px-4 flex items-center gap-3 w-full">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                         ...
                         <div className="font-medium text-slate-800">
                            {data.date ? format(new Date(data.date), "d MMMM yyyy", { locale: th }) : '-'}
                         </div>
                     </div>
                     <div>
      ...
      <div className="shrink-0 p-4 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-[60] flex gap-3">
                <Button 
