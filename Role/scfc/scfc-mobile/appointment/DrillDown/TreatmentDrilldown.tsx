...
import {
  ...
  Building2, Stethoscope, Activity, MapPin, PieChart as PieChartIcon,
} from 'lucide-react';
...
const TREATMENT_COLORS = ['#49358E', '#7066A9', '#ff9f43', '#28c76f', '#ea5455', '#00cfe8', '#9b59b6', '#e67e22'];
...
export function TreatmentDrilldown({ appointments, treatmentName, treatmentColor, onBack, onSelectAppointment }: Props) {
  const allTreatments = useMemo(() => {
    ...
    appointments.forEach(a => { const raw = (a.type || a.clinic || 'ไม่ระบุ').trim(); map.set(TREATMENT_MAP[raw] || raw, (map.get(TREATMENT_MAP[raw] || raw) || 0) + 1); });
    return Array.from(map.entries()).map(([name, value], i) => ({ name, value, color: TREATMENT_COLORS[i % TREATMENT_COLORS.length] })).sort((a, b) => b.value - a.value).slice(0, 8);
  }, [appointments]);

  const [selectedName, setSelectedName] = useState(treatmentName);
  const selectedTreatment = allTreatments.find(t => t.name === selectedName) || allTreatments[0];
  const activeColor = selectedTreatment?.color || treatmentColor;

  const filtered = useMemo(() => {
    if (!selectedName) return appointments;
    return appointments.filter(a => { const raw = (a.type || a.clinic || 'ไม่ระบุ').trim(); return (TREATMENT_MAP[raw] || raw) === selectedName; });
  }, [appointments, selectedName]);
  ...
  const hospitalData = useMemo(() => {
    const map = new Map<string, number>();
    filtered.forEach(a => { const h = (a.hospital || 'ไม่ระบุ').replace('โรงพยาบาล', 'รพ.').trim(); map.set(h, (map.get(h) || 0) + 1); });
    return Array.from(map.entries()).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
  }, [filtered]);

  const maxTreatmentVal = allTreatments[0]?.value || 1;
    ...
    <div className="bg-[#F4F0FF]/40 min-h-screen flex flex-col font-['IBM_Plex_Sans_Thai'] pb-20">
      <div className="sticky top-0 w-full bg-[#7066a9] h-[64px] px-4 flex items-center gap-3 shrink-0 z-50 shadow-md">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-white text-[18px] font-bold truncate">ตามประเภทการรักษา</h1>
          <p className="text-white/70 text-[14px] truncate flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full shrink-0 inline-block" style={{ backgroundColor: activeColor }}></span>{selectedName}</p>
        </div>
        <span className="text-white text-[14px] font-bold bg-white/20 px-3 py-1 rounded-full shrink-0">{filtered.length} นัดหมาย</span>
      </div>
      ...
      <div className="p-4 space-y-4 flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] overflow-hidden">
          <div className="p-4 border-b border-[#F4F0FF] flex items-center justify-between"><h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2"><Activity className="text-[#7066A9]" size={18} /> เลือกประเภทการรักษา</h3><span className="text-[14px] font-bold text-white bg-[#49358E] px-2 py-0.5 rounded-full">{allTreatments.length} ประเภท</span></div>
          <div className="divide-y divide-[#F4F0FF]">
            {allTreatments.map(tt => {
              const isActive = tt.name === selectedName;
              const pct = Math.round((tt.value / maxTreatmentVal) * 100);
              return (
                <button key={tt.name} onClick={() => setSelectedName(tt.name)} className={cn("w-full text-left px-4 py-3 transition-all", isActive ? "bg-[#F4F0FF]" : "hover:bg-[#F4F0FF]/30")}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2.5">
                      <div className={cn("w-4 h-4 rounded-full shrink-0 flex items-center justify-center", isActive && "ring-2 ring-offset-1")} style={{ backgroundColor: tt.color }}>
                        {isActive && <CheckCircle2 size={10} className="text-white" />}
                      </div>
                      <span className={cn("text-[16px]", isActive ? "font-bold text-[#37286A]" : "text-[#5e5873]")}>{tt.name}</span>
                    </div>
                    <span className={cn("text-[18px] font-black", isActive ? "text-[#37286A]" : "text-[#5e5873]")}>{tt.value}</span>
                  </div>
                  <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden"><div className={cn("h-full rounded-full transition-all duration-500", isActive ? "opacity-100" : "opacity-60")} style={{ width: `${pct}%`, backgroundColor: tt.color }}></div></div>
                </button>
              );
            })}
          </div>
        ...
        >
          {[
            { label: 'ทั้งหมด', value: stats.total, icon: Activity, iconColor: 'text-[#49358E]', iconBg: 'bg-[#F4F0FF]', border: 'border-[#E3E0F0]' },
            ...
            { label: 'เสร็จสิ้น', value: stats.completed, icon: CheckCircle2, iconColor: 'text-emerald-600', iconBg: 'bg-emerald-50', border: 'border-emerald-100' },
          ].map((s, i) => (
        ...
        {pieData.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] p-4">
            <h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2 mb-3"><PieChartIcon className="text-[#7066A9]" size={18} /> สัดส่วนสถานะ — {selectedName}</h3>
        ...
        {hospitalData.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-[#E3E0F0] p-4">
            <h3 className="font-bold text-[#37286A] text-[18px] flex items-center gap-2 mb-3"><Building2 className="text-[#7066A9]" size={18} /> แยกตามโรงพยาบาล</h3>
            <div className="space-y-3">
              {hospitalData.map(h => {
                const pct = Math.round((h.value / (hospitalData[0]?.value || 1)) * 100);
                return (
                  <div key={h.name} className="space-y-1.5">
                    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Building2 size={16} className="text-[#49358E]" /><span className="text-[16px] text-[#37286A]">{h.name}</span></div><span className="text-[16px] font-black text-[#37286A]">{h.value}</span></div>
                    <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: activeColor }}></div></div>
                  </div>
                );
              })}
            </div>
        ...
        <div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-[#E3E0F0] text-[#7066A9]">
              ...
              <p className="text-[16px]">ไม่พบนัดหมายประเภท {selectedName}</p>
            </div>
          ) : (
            <div className="space-y-3">
